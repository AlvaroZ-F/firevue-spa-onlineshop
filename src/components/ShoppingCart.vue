<template>
	<div class="container table-responsive">
		...
		<table id="cart" class="table-responsive table-hover table-sm">
			...
			<tfoot>
				<tr class="d-table-row d-sm-none">
					<td class="text-center">
						<strong>Total {{ cartValue }} &euro;</strong>
					</td>
				</tr>
				<tr>
					<td>
		...
		methods: {
			...
			checkValidCart(itemList, prodList) {
				...
				itemList.map((item) => {
					for (let i = 0; i < prodList.length; i++) {
						if (prodList[i].id == item.id) {
							if (prodList[i].quantity < item.quantity) {
								message = `Only ${prodList[i].quantity} ${item.title} available in stock`
								isValid = false
								return
							}
							break
			...
			checkout() {
				if (this.isLoggedIn) {
					...
					let { isValid, messsage } = this.checkValidCart(
						this.cardItemList,
						this.products
					)
